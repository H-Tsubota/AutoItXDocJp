...
//		- 貢献者については "AUTHORS.txt" を参照
//
// このファイルはAutoItXの一部です。 このファイルとAutoItX DLLの利用時には
// ヘルプファイルに詳細のあるAutoItXライセンスの条件に従う必要があります。
//
// AutoItX3.dllを通常のDLLとして使用する場合、DLLとAutoItX3.libの利用に必要な
// 定義と宣言はこのファイルに記載されています。
//
///////////////////////////////////////////////////////////////////////////////
...
// 定義
#define AU3_INTDEFAULT			(-2147483647)	// _一部_のパラメータの"デフォルト"値 (最小の負数)

//
// 注意: このDLLではUnicode文字列を使用しています。 ほんの一部の関数は
// ANSIバージョンも提供されています。ANSI文字列を使用した場合は古いバージョンのAutoItXを使うか
// その問題についてフォーラム http://www.autoitscript.com/forum で聞いてください。
//

//
// nBufSize
// 結果文字列バッファのサイズ指定に使用する際、この値はバッファのnull終端文字を含む文字数です。
// 例:
//
// WCHAR szBuffer[10];
// AU3_ClipGetW(szBuffer, 10);
//
// 結果文字列は10文字目にnull終端が設定され、9文字に切り詰められます。
//
